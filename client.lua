...
Citizen.CreateThread(function()
	while true do
		...
		if #(pos - Config.shop["coords"]) < 5.0 then
			inRange = true
			DrawText3Ds(Config.shop["coords"], "[~g~E~w~] Магазин")
		...
		if active_machines ~= nil then
			for k, v in pairs(active_machines) do
				if #(pos - v.coords) < 3.0 then
					inRange = true
					DrawText3Ds(v.coords, "Оставащо време: ~r~"..v.time)
					DrawText3Ds(vector3(v.coords.x, v.coords.y, v.coords.z+0.3), "Машина: ~b~"..v.name)
				end
			end
		end
		...
		if #(pos - Config.MiningLab["coords"]) < 3.0 then
			inRange = true
			DrawText3Ds(Config.MiningLab["coords"], "[~r~E~w~] Изход")
			if IsControlJustPressed(0, 38) then
		...
		if #(pos - vector3(Config.ExitCoords.x,Config.ExitCoords.y,Config.ExitCoords.z)) < 3.0 then
			inRange = true
			DrawText3Ds(Config.ExitCoords, "[~g~E~w~] Вход")
			if IsControlJustPressed(0, 38) then
		...
		if active_machines ~= nil then
			for k, v in pairs(active_machines) do
				if v.time > 0 then
					...
					TriggerServerEvent("nd-cryptomining:server:addCryptoCoins", v.reward)
...
RegisterNetEvent("nd-cryptomining:client:installCPU", function(name, reward, item)
	local ped = PlayerPedId()
	local pos = GetEntityCoords(ped)
	if #(pos - Config.MiningLab["coords"]) < 50.0 then
		TriggerEvent('ultra-voltlab', 60, function(result)
			if result == 0 then
				NDCore.Functions.Notify('Неуспешно инсталиране','error')
			elseif result == 1 then
				InstallCPU(name, reward, item.name)
				NDCore.Functions.Notify('Добра работа!', 'success')
			else
				NDCore.Functions.Notify('Нещо се обърка','error')
			end
		end)
	end
end)
...
function InstallCPU(name, reward, itemname)
	if #active_machines == 0 then
		...
		local machine = {}
		NDCore.Functions.Progressbar("cpuinstall", "Инсталиране...", 10000, false, true, {
			...
			NDCore.Functions.Notify('Инсталацията е успешна','success')
			TriggerServerEvent("nd-cryptomining:server:RemoveItem", itemname)
		end, function() -- Cancel
			ClearPedTasks(ped)
			NDCore.Functions.Notify("Инсталацията спря..", "error")
		end)
	else
		NDCore.Functions.Notify("Вече имате работеща машина", "error")
	end
end
